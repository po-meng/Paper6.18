...
# =========================================================
# 代码 1：复现 Fig.8 / Fig.9
# 功能：10% 风电渗透率（18 台 WTG），比较 100/10/6/5/2 step Common Wind Model
# 地点：Swift Current、Toronto
# =========================================================


def run_step_number_case(site_name: str, wtg_count: int = 18) -> pd.DataFrame:
    mu, sigma = read_actual_mu_sigma(site_name)
    print(f"\n========== {site_name}：10% wind penetration，WTG = {wtg_count} ==========")
    ...
    for nb in STEP_LIST:
        ...
        wind_path = OUT_DIR / f"Fig8_9_{safe_name(site_name)}_{nb}step_wind_model.csv"
        wind_df.to_csv(wind_path, index=False, encoding="utf-8-sig")

        power_model = wind_model_to_power_model(
            wind_df=wind_df,
            wtg_count=wtg_count,
            model_name=f"Fig8_9_{safe_name(site_name)}_{nb}step_{wtg_count}WTG",
            out_dir=OUT_DIR,
            save_detail=True,
        )

        for peak_pu in PEAK_LOAD_PU_LIST:
            lole_value = calculate_lole_for_wind_power_model(
                conv_copt=conv_copt,
                ldc_df=ldc_df,
            ...
            rows.append({
                ...
                "wind_penetration_label": "10%",
                "step_number": nb,
                "peak_load_pu": peak_pu,
                "LOLE_hours_per_year": lole_value,
            })

    result = pd.DataFrame(rows)
    csv_path = OUT_DIR / f"Fig8_9_{safe_name(site_name)}_10pct_step_LOLE.csv"
    result.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"{site_name} 10% step-LOLE 数据已保存：{csv_path}")
    ...
    pivot_path = OUT_DIR / f"Fig8_9_{safe_name(site_name)}_10pct_step_LOLE_pivot.csv"
    pivot.to_csv(pivot_path, index=False, encoding="utf-8-sig")

    fig_name = "Fig8" if site_name == "Swift Current" else "Fig9"
    fig_path = OUT_DIR / f"{fig_name}_{safe_name(site_name)}_10pct_step_LOLE.png"
    y_cols = [f"{nb}-step" for nb in STEP_LIST]
    plot_lines(
        df=pivot,
        x_col="peak_load_pu",
        y_cols=y_cols,
        title=f"{fig_name}: {site_name} wind site with 10% wind penetration ({wtg_count} WTG)",
        xlabel="Peak Load (p.u. of RBTS peak load)",
        ylabel="LOLE (hours/year)",
        fig_path=fig_path,
    )
    print(f"{fig_name} 图像已保存：{fig_path}")
    print(pivot.round(4).to_string(index=False))
    return result


def main():
    print("========== 代码1：复现 Fig.8 / Fig.9：10% 风电渗透率下不同 step 数 LOLE 对比 ==========")
    print(f"输出目录：{OUT_DIR}")
    check_required_files(require_fig2=False, require_common=True, require_summary=True)

    all_results = []
    for site in ["Swift Current", "Toronto"]:
        all_results.append(run_step_number_case(site_name=site, wtg_count=18))

    final_df = pd.concat(all_results, ignore_index=True)
    final_path = OUT_DIR / "Fig8_Fig9_10pct_all_sites_step_LOLE.csv"
    final_df.to_csv(final_path, index=False, encoding="utf-8-sig")
    print("\n========== 完成 ==========")
    print(f"Fig.8 / Fig.9 汇总结果已保存：{final_path}")
