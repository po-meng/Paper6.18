...
# =========================================================
# 代码 3：复现 Six-step Common Wind Speed Model、Table VI、Table VII、Fig.11
# 功能：按照论文式(12)生成 6-step 通用风速模型，并以 Regina 为例生成风速表和功率表。
# =========================================================


def build_six_step_common_model() -> pd.DataFrame:
    base = load_common_wind_model_100step()
    six = rebin_probability_model(base, 6, prob_col="probability")
    six["probability"] = six["probability"] / six["probability"].sum()
    six_path = OUT_DIR / "Fig11_SixStep_Common_Wind_Model_standardized.csv"
    six.to_csv(six_path, index=False, encoding="utf-8-sig")
    print(f"6-step 标准化通用风速模型已保存：{six_path}")
    return six


def plot_fig11(six_model: pd.DataFrame):
    fig_path = OUT_DIR / "Fig11_SixStep_Common_Wind_Speed_Model.png"
    plt.figure(figsize=(7.5, 4.8))
    plt.bar(six_model["z_midpoint"], six_model["probability"], width=1.0, align="center", alpha=0.8)
    plt.xlabel("Simulated Wind Speed by Mean Value (μ) and Standard Deviation (σ)")
    plt.ylabel("Probability")
    plt.title("Fig.11 Six-step Common Wind Speed Model")
    plt.grid(True, axis="y", linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig(fig_path, dpi=300)
    plt.close()
    print(f"Fig.11 图像已保存：{fig_path}")


def build_table_vi_for_regina(six_model: pd.DataFrame) -> pd.DataFrame:
    regina_wind = build_common_wind_model_for_site(six_model, REGINA_MU_KMH, REGINA_SIGMA_KMH)
    regina_wind = regina_wind.rename(columns={
        "step_index": "Step",
        "z_midpoint": "Wind Speed expressed by mu_sigma",
        "wind_speed_raw_kmh": "Wind Speed raw km/h",
        "wind_speed_kmh": "Wind Speed km/h negative converted to zero",
        "probability": "Probability",
    })

    # 添加便于对照论文式(12)的表达
    labels = []
    for i in regina_wind["Step"]:
        coef = (i - 3) * 5 / 3
        if abs(coef) < 1e-12:
            labels.append("mu")
        elif coef > 0:
            labels.append(f"mu + {coef:.4f} sigma")
        else:
            labels.append(f"mu - {abs(coef):.4f} sigma")
    regina_wind.insert(2, "Expression", labels)

    path = OUT_DIR / "TableVI_Regina_SixStep_Wind_Model.csv"
    regina_wind.to_csv(path, index=False, encoding="utf-8-sig")
    print(f"Table VI Regina 六步风速模型已保存：{path}")
    print(regina_wind.round(6).to_string(index=False))
    return regina_wind


def build_table_vii_for_regina(six_model: pd.DataFrame, wtg_count: int = 18) -> pd.DataFrame:
    regina_wind = build_common_wind_model_for_site(six_model, REGINA_MU_KMH, REGINA_SIGMA_KMH)
    power_model = wind_model_to_power_model(
        wind_df=regina_wind,
        wtg_count=wtg_count,
        model_name=f"TableVII_Regina_Simplified_{wtg_count}WTG",
        out_dir=OUT_DIR,
        save_detail=True,
    )
    table_vii = power_model[["wind_farm_power_MW", "probability", "wind_state_count", "wind_speed_min_kmh", "wind_speed_max_kmh"]].copy()
    table_vii = table_vii.rename(columns={
        "wind_farm_power_MW": "Power Output MW",
        "probability": "Probability",
        "wind_state_count": "Number of six-step wind states combined",
        "wind_speed_min_kmh": "Min wind speed km/h",
        "wind_speed_max_kmh": "Max wind speed km/h",
    })
    path = OUT_DIR / "TableVII_Regina_Simplified_Power_Generation_Model.csv"
    table_vii.to_csv(path, index=False, encoding="utf-8-sig")
    print(f"Table VII Regina 简化风电场功率模型已保存：{path}")
    print(table_vii.round(6).to_string(index=False))
    return table_vii


def main():
    print("========== 代码3：复现 Fig.11 / Table VI / Table VII：六步简化通用风速与功率模型 ==========")
    print(f"输出目录：{OUT_DIR}")
    print(f"Regina 参数：mu = {REGINA_MU_KMH} km/h, sigma = {REGINA_SIGMA_KMH} km/h")
    check_required_files(require_fig2=False, require_common=True, require_summary=False)

    six_model = build_six_step_common_model()
    plot_fig11(six_model)
    build_table_vi_for_regina(six_model)
    build_table_vii_for_regina(six_model, wtg_count=18)
