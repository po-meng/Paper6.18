...
SITE_NAME = "TorontoIslandA"

DATA_DIR = Path(r"D:\tools\za\paper\Toronto Island A_cleaned")
OUT_DIR = Path(r"D:\tools\za\paper\Toronto Island A_Refit_ARMA_Result")
...
def build_standardized_y_series(data: pd.DataFrame, clim: pd.DataFrame):
    ...
    return merged["y_t"].to_numpy(), merged
...
def extract_arma_params(result):
    ...
    ar_coefs = [
        float(param_dict.get(f"ar.L{i}", 0.0))
        for i in range(1, AR_ORDER + 1)
    ]

    ma_coefs = [
        float(param_dict.get(f"ma.L{i}", 0.0))
    ...
    return ar_coefs, ma_coefs, sigma_alpha, param_table
...
def simulate_wind_speed(mu_vec, sigma_vec, ar_coefs, ma_coefs, sigma_alpha, n_years: int):
...
def save_statistics_and_outputs(data, mu_vec, sigma_vec, y_sim, sw_raw, sw_zero, n_years):
    ...
    plot_probability_distribution(
        x=x_raw,
        p=p_raw,
        title=f"Probability Distribution of Simulated Wind Data - {SITE_NAME} Refit ARMA",
        save_path=fig_raw_path,
        label=f"{SITE_NAME} Refit ARMA",
        x_min=-20,
        x_max=70
    )
    ...
    plot_probability_distribution(
        x=x_zero,
        p=p_zero,
        title=f"Probability Distribution after Negative Wind Speeds Set to Zero - {SITE_NAME} Refit ARMA",
...
def main():
    ...
    y_fit, y_fit_df = build_standardized_y_series(data, clim)

    result = fit_arma_model(y_fit)

    ar_coefs, ma_coefs, sigma_alpha, param_table = extract_arma_params(result)
